...
3. Aplicar a correção mínima necessária, sem refatorar código não relacionado ao bug.
4. Confirmar que o bug não ocorre mais, repetindo os mesmos passos de reprodução.
5. Verificar rapidamente fluxos vizinhos que dependem do mesmo estado (ex.: se corrigiu `ManagedResourceControls`, conferir `SpellsPanel` e `ResourcesPanel`).
