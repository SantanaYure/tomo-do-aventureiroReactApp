...
4. Se a mudança envolve UI, iniciar `npm run dev` e testar manualmente o caminho principal no navegador.
5. Rodar `npx tsc --noEmit` e corrigir erros de tipo introduzidos.
6. Rodar `npm run lint` (cobre apenas `.js`/`.jsx`; não substitui a checagem de tipos acima).
7. Registrar resultado, arquivos alterados e qualquer pendência.

## Verificação

- Teste manual no navegador do caminho principal da funcionalidade.
...
- Evidências de verificação (o que foi testado manualmente, resultado de `tsc` e `lint`).
